class PingvinavisaBehavior {
  ...
  // Hjelpefunksjon for sleep som fungerer uansett om ctx.Lib er tilgjengelig eller ikke
  async sleep(ctx, ms) {
  ...
  // Hjelpefunksjon for å legge til lenker i køen som fungerer uansett Browsertrix-versjon
  addLink(ctx, url) {
  ...
  // Hjelpefunksjon for å oppdatere og hente tilstand
  getState(ctx, msg, key) {
  ...
  async clickLoadMore(ctx) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (text.includes("vis flere")) {
        ...
        await this.sleep(ctx, 4000); // Gir React tid til å tegne opp nye artikler
        return true;
      }
    }
    return false;
  ...
  async *run(ctx) {
    ...
    for (var i = 0; i < maxAttempts; i++) {
      ...
      if (clicked) {
        ...
        yield this.getState(ctx, "Klikket 'Vis flere', totalt funnet: " + seenUrls.size + " artikler", "clicks");
        continue; 
      }

      window.scrollTo({ top: document.documentElement.scrollHeight, behavior: "smooth" });
      await this.sleep(ctx, 2000); 
      ...
      ctx.state.scrolls++;
      yield this.getState(ctx, "Scrollet. Artikler i innhøstingskø: " + seenUrls.size, "articlesFound");
    }
    
    yield this.getState(ctx, "Behavior-script ferdig. Crawleren tar nå over for å besøke alle artiklene!");
  }
}
